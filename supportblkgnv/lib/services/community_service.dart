...
class CommunityService {
  ...
  Future<CommunityGoal?> getCommunityGoalById(String goalId) async {
    ...
    try {
      final doc = await _firestore.collection(goalsCollection).doc(goalId).get();
  ...
  Future<CommunityImpactReport?> getLatestImpactReport() async {
    ...
    try {
      ...
      // Get top businesses
      final List<dynamic> topBusinessIds = reportDoc.data()['topBusinessIds'] ?? [];
      ...
      // Get completed goals
      final List<dynamic> completedGoalIds = reportDoc.data()['completedGoalIds'] ?? [];
      ...
      categorySpending.forEach((key, value) {
        spendingByCategory[_parseBusinessCategory(key)] = (value as num).toDouble();
      });
