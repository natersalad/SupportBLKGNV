...
import 'package:supportblkgnv/services/storage_service.dart';
...
class PostService {
  ...
  Future<List<Post>> getPosts() async {
    ...
    try {
      ...
      final allDocs =
          batchSnapshots.expand((snap) => snap.docs).toList()..sort((a, b) {
            // Safely parse createdAt from different possible formats
            DateTime getDateTime(dynamic createdAt) {
              if (createdAt is Timestamp) {
                ...
                try {
                  ...
                  // If string can't be parsed, use epoch time
                  return DateTime(1970);
                }
              } else {
                // Default to current time if neither timestamp nor string
                return DateTime.now();
              }
            }

            final ta = getDateTime(a.data()['createdAt']);
            final tb = getDateTime(b.data()['createdAt']);
      ...
      for (final doc in uniqueDocs) {
        ...
        for (var commentDoc in commentsSnapshot.docs) {
          ...
          if (userCache.containsKey(commentUserId)) {
            ...
            // Fetch user
            final userDoc =
                await _firestore.collection('users').doc(commentUserId).get();
            if (userDoc.exists) {
  ...
  Future<Post?> createPost(
    ...
    try {
      // Handle image upload if there's a local file path
      String? remoteImageUrl = imageUrl;
      if (imageUrl != null && !imageUrl.startsWith('http')) {
        // This is a local file path, upload to Firebase Storage
        final imageFile = File(imageUrl);
        remoteImageUrl = await _storageService.uploadImage(imageFile);
      ...
      // Create post in Firestore with the remote URL
      final postData = {
        'authorId': author.id,
        'content': content,
        'imageUrl': remoteImageUrl, // Now this is a Firebase Storage URL
